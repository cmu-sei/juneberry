...
for d in [TRAIN_ROOT, TEST_ROOT]:
    for lab in CIFAR100_LABELS:
        os.makedirs(os.path.join(os.path.join(DEST_ROOT, d, lab)), exist_ok=True)

# Items that need to be in a train_spec file
train_spec_filename = "cifar100_32x32_rgb_train_van.json"
train_spec = {}
train_spec['numModelClasses'] = 100
...
train_spec["data"] = [{"label": i, "labelName": label, "directory": os.path.join(DIR_NAME, "train", label)} for i, label
                      in enumerate(CIFAR100_LABELS)]

test_spec_filename = "cifar100_32x32_rgb_test_van.json"
test_spec = {}
test_spec['numModelClasses'] = 100
test_spec['imageProperties'] = {"dimensions": "32,32", "colorspace": "rgb"}
test_spec['sampling'] = {"algorithm": "none", "arguments": {}}
test_spec["description"] = "cifar100 test images, 32x32, rgb"
...
for img, label in train_set:
    name = f"{cnt:0>6d}.png"
    img.save(os.path.join(DEST_ROOT, TRAIN_ROOT, CIFAR100_LABELS[label], name))
    cnt += 1

for img, label in test_set:
    name = f"{cnt:0>6d}.png"
    img.save(os.path.join(DEST_ROOT, TEST_ROOT, CIFAR100_LABELS[label], name))
    cnt += 1

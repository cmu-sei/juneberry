...
class AttackMaker:
    ...
    def __init__(self, args):
        ...
        self.attack_config = jb_attack.AttackConfig()
        ...
        # Setup the experiment directory for the attack.
        self.attack_manager.setup()

        # Set logging properties depending on whether or not dry run mode was requested.
        ...
        jbscripting.setup_for_experiment_creation(args, self.attack_manager, log_file=log_file, log_prefix=log_prefix)
    ...
    def _add_private_model_training_rule(self, workflow, disjoint=False):
        ...
        private_model_arg = self.attack_config.watermarks.private_disjoint_args if disjoint \
            else self.attack_config.watermarks.private_superset_args
        universe = self.attack_config.watermarks.disjoint_args if disjoint \
            else self.attack_config.watermarks.superset_args
        ...
        outputs = [str(x) for x in model_mgr.get_training_output_list()]
    ...
    def _add_shadow_model_rule(self, workflow, disjoint=False):
        """
        ...
        shadow_eval_output_file_list = self.shadow_disjoint_eval_output_files_list if disjoint \
        ...
        for i in range(num_shadow_models):
            ...
            outputs = shadow_model_mgr.get_training_output_list()
    ...
    def _add_meta_model_rule(self, workflow, disjoint=False):
        ...
        oppo_test_cfg = str(self.attack_manager.get_experiment_inout_dataset_path('test', disjoint=(not disjoint)))
        ...
        plugin_file = self.attack_manager.get_meta_plugin_file(disjoint=disjoint)
        ...
        logger.info(f"*** Adding a rule to build the {universe_str} in_out datasets ***")
        ...
        workflow.add_rule(inputs, outputs, command, doc, clean_extras, requirements)
        logger.info(f"*** Added the rule to build the in_out datasets. ***\n")
        ...
        outputs = meta_model_mgr.get_training_output_list()
        ...
        # Two evaluations of the trained meta model are possible: one for the test dataset in the current universe and
        # one for the test dataset in the opposite universe. Build a rule for each evaluation.
        test_datasets = [test_cfg_dest, oppo_test_cfg]
